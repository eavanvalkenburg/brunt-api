...
class BruntAPI:
    ...
    def _is_logged_in(self):
        ...
        if not self._http._has_session() or datetime.utcnow() >= self._lastlogin + timedelta(
            hours=24
        ):
            return self._login()
        else:
    ...
    def getThings(self):
        ...
        data = {"path": "/thing", "host": "https://sky.brunt.co"}
    ...
    def getState(self, **kwargs):
        ...
        data = {"path": f"/thing{thingUri}", "host": "https://thing.brunt.co:8080"}
        resp = self._http.request(data, RequestTypes.GET)
        resp.update(login_return)
        return resp
    ...
    def changeKey(self, **kwargs):
        ...
        if "thingUri" in kwargs:
            ...
            return SyntaxError(
                "Please provide either the 'thing' name or the 'thingUri' not both and at least one"
            )
